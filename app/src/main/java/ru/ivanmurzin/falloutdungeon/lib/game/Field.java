...
public class Field {
    ...
    public static class Level {
        ...
        public Level(int levelNumber, int fieldSize, Door door) {
        ...
        public void update() {
            if (units.size() == 1 && door.getState() == DoorState.Closed) {
                door.open();
            }
            ...
            while (unitIterator.hasNext()) {
                ...
                if (isOutOfBorders(unit)) {
                    if (unit.x > (fieldSize - 5) * TILE_SIZE)
                        unit.x = (fieldSize - 5) * TILE_SIZE;
                    else if (unit.x < 0) unit.x = 0;
                    else if (unit.y > (fieldSize - 5) * TILE_SIZE)
                        unit.y = (fieldSize - 5) * TILE_SIZE;
                    else unit.y = 0;
                }
